#!/usr/bin/env python3
"""overlay.py — Step 3 of the overlay pipeline (Approach B: 2-block).

Wraps MECH in MECH_DRAWING and STR in STR_DRAWING (preserving original
coords), then places 2 INSERTs at modelspace. The gridline-derived
affine lives on the `STR_DRAWING` INSERT's (insert, xscale, yscale) —
no per-entity transform needed.

Structure:
  MECH_DRAWING block (red, ACI 1)  — MECH entities at original MECH coords
  STR_DRAWING  block (blue, ACI 5) — STR entities at original STR coords
  Modelspace:
    INSERT MECH_DRAWING  pos=(0, 0)        xscale=1
    INSERT STR_DRAWING   pos=(tx, ty)      xscale=sx, yscale=sy

Editability: MEDIUM — click STR_DRAWING to grab the whole layer;
double-click into STR_DRAWING to edit individual entities.
...
OUT = OUT_DIR / "TP-104 OVERLAY_B.dxf"
...
def main():
    print("=" * 70)
    print("Step 3: OVERLAY — Approach B (2-block)")
    print("=" * 70)
